...
const LinzaSotuvi = () => {
  ...
  const handleDelete = useCallback(async () => {
    ...
    if (isOperationPending(`linza-sotuvi-delete-${deleteId}`)) {
      return;
    }

    await guardOperation(async () => {
      return await withDuplicatePrevention(`linza-sotuvi-delete-${deleteId}`, async () => {
        setIsDeleting(true);
  ...
  }, [deleteId, user, isDeleting, isOperationPending, guardOperation, withDuplicatePrevention, sotuvlar, t]);
  ...
  const handleUpdate = useCallback(async (e: React.FormEvent) => {
    ...
    if (isOperationPending(`linza-sotuvi-update-${editingItem.id}`)) {
      return;
    }

    await guardOperation(async () => {
      return await withDuplicatePrevention(`linza-sotuvi-update-${editingItem.id}`, async () => {
        setIsUpdating(true);
        try {
          const { error } = await supabase
            .from("linza_sotuvlari")
  ...
  const handleExportToExcel = async () => {
    ...
    try {
      await exportDataToExcel({
        fileName: `Linza_Sotuvi_${formatUzbekistanDate()}.xlsx`,
