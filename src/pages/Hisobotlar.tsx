...
const Hisobotlar = () => {
  ...
  const loadReportData = () => {
    ...
    let allData = [...buyurtmalar.map((b: any) => ({
      sana: b.sana,
      summa: b.jamiSumma,
      tur: "Buyurtmalar"
  ...
  const exportToExcel = (type: "period" | "section" | "detailed") => {
    ...
    if (type === "period") {
      ...
      const allData = [...buyurtmalar.map((b: any) => ({
  ...
  const exportToPDF = (type: "period" | "section" | "detailed") => {
    ...
    doc.setFontSize(10);
    doc.text(`${t("common.date")}: ${new Date().toLocaleDateString("uz-UZ", {
    ...
    if (type === "period") {
      ...
      const sections = [[t("nav.orders"), buyurtmalar.reduce((sum: number, b: any) => sum + b.jamiSumma, 0)], [t("nav.examination"), tekshiruvlar.reduce((sum: number, t: any) => sum + t.jamiSumma, 0)], [t("nav.readyGlasses"), tayyorKozoynaklar.reduce((sum: number, k: any) => sum + k.summa, 0)], [t("nav.lensSales"), linzaSotuvlari.reduce((sum: number, l: any) => sum + l.summa, 0)]];
      ...
      const allData = [...buyurtmalar.map((b: any) => [t("nav.orders"), b.sana, b.mijoz, b.jamiSumma.toLocaleString()]), ...tekshiruvlar.map((tek: any) => [t("nav.examination"), tek.sana, tek.mijoz, tek.jamiSumma.toLocaleString()]), ...tayyorKozoynaklar.map((k: any) => [t("nav.readyGlasses"), k.sana, k.mijoz, k.summa.toLocaleString()]), ...linzaSotuvlari.map((l: any) => [t("nav.lensSales"), l.sana, l.mijoz, l.summa.toLocaleString()])];
      autoTable(doc, {
        startY: 30,
  ...
  return <div className="space-y-6">
            ...
            <div className="h-[400px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={reportData}>
                  ...
                  <XAxis dataKey="name" />
                  <YAxis />
                  <Tooltip formatter={(value: number) => `${value.toLocaleString()} ${t("common.sum")}`} contentStyle={{
      ...
      <Card className="p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          name: "Tayyor ko'zoynaklar",
          key: "tayyorKozoynaklar",
          field: "summa"
        }, {
          name: "Linza sotuvi",
          key: "linzaSotuvlari",
          ...
          return <div key={section.key} className="bg-secondary rounded-lg p-4">
